...
#   тестовой группе и столько же в контрольной.
# - Тестирование показало [статистически значимую разницу](#ssi_test_pvalue)
#   в конверсии между группами.
...
# всего 0.8 __процентного пункта__, это рост конверсии на _25%_ (${0.4-0.32})/{0.32}$.
#
# Согласно общепринятому стандарту, нужно показать, что мы с вероятностью 80% заметим изменение,
# причём оно будет стастически достоверно на уровне 95%.
#
...
#   - За время тестов поведение клиента не изменяется (предположение статичной ситуации)
...
# Входные данные:
# 
#  - Тестовая группа: 8732 клиента, 293 повторных покупки (=успеха).
#  - Контрольная группа: 8847 клиентов, 347 повторных покупок.
...
test_g = Group(size=8732, success=293, conv_rate=293/8732, stderr=get_sterr(8732, 293))
ctrl_g = Group(size=8847, success=347, conv_rate=347/8847, stderr=get_sterr(8847, 347))
...
Доверительные интервалы 95%:
    Контрольная группа: от {0:.4f} до {1:.4f},
    Тестовая группа:    от {2:.4f} до {3:.4f}.
