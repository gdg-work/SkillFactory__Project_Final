...
# %% [markdown]
# Разработана программа, которая по идентификатору (ID) курса предлагает ещё два ID курсов
# для допродажи.  Рекомендуемые курсы выбираются на основании таблицы частоты встречаемости
# пар курсов.
#
...
#
# При запуске программы `final_project_pt1.py` построенная на основании текущих данных в базе
# данных (БД) таблица выдаётся на стандартный вывод в CSV формате.  Возможна модификация
# программы для выдачи других форматов или работы в составе API.
#
# %% [markdown]
# ### Принцип работы <a name='res.principle'/>
# %% [markdown]
# На основании истории продаж, записанной в базе данных, каждому курсу $ID_k$ ставится в
# соответствие таблица из идентификаторов других курсов $ID_n$ и количества раз, когда
# эти курсы ($ID_n$) были проданы тому же пользователю, который приобретал курс $ID_k$. эта
# таблица сортируется по убыванию частоты встречаемости курсов $ID_n$, после чего для
# выдачи рекомендаций используются два наиболее часто встречающихся курса из $ID_n$.
#
# %% [markdown]
# ### Принятые компромиссы и ограничения <a name='res.limitations'/>
# %% [markdown]
# 1. Если среди $ID_n$ нет курсов, которые продаются достаточно часто, то в качестве
# рекомендаций будут выданы наиболее часто продаваемые курсы из всей совокупности.
# Таким образом, в рекомендациях к самым редко покупаемым курсам окажутся лидеры продаж.
#
# 2. «Достаточно часто» в предыдущем пункте — это в верхних 90% общего списка продаваемых
# курсов, отсортированного по убыванию частоты продажи.
#
# 3. Новые курсы, ещё не набравшие необходимого количества продаж (см. пункт 2), не будут показываться
# в рекомендациях, таким образом, рекомендательная система может затруднить продажи новых курсов.
#
# %% [markdown]
...
# %% [raw]
# Эта ячейка намеренно сделана типа 'raw', чтобы не выполняться в Jupyter Notebook
if __name__ == "__main__" and not INTERACTIVE:
    # called as a standalone program, so print recommendations list and exit
    print(get_recommended_courses(course_pairs_df, unpopular_threshold, freq_table).sort_index().to_csv())
